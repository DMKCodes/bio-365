import imperviousBg from '../media/impervious-video.mp4';
import learnBg from '../media/learn-page-video.mp4';
import homeBg from '../media/home-page-video.mp4';
import newsBg from '../media/news-page-video.mp4';
import storiesBg from '../media/story-page-video.mp4';
import resourceBg from '../media/resource-page-video.mp4';
import glossaryBg from '../media/glossary-page-video.mp4';

export const HOME_PAGE_VIDEO_BG = {
    video: homeBg,
    videoSource: null,
    title: 'BIODIVERSITY MADE SIMPLE',
    content: [
        "Our planet's wildlife faces unprecedented risk, and human activity is to blame.",
        'Dive in to learn more about our incredible natural world, the challenges it faces, and what we can do to help.',
        'Explore, learn, protect — today and every day.'
    ],
    author: null,
    button: '',
    buttonLink: ''
};

export const LEARN_PAGE_VIDEO_BG = {
    video: learnBg,
    videoSource: null,
    title: 'WHAT IS BIODIVERSITY?',
    content: [
        'Biodiversity is the term we use to describe all the different kinds of life on Earth—the variety of animals, plants, fungi, and even microorganisms like bacteria that make up our natural world. These species work together in ecosystems, like an intricate web, to maintain balance and support life.'
    ],
    author: null,
    button: null,
    buttonLink: null
};

export const NEWS_PAGE_VIDEO_BG = {
    video: newsBg,
    videoSource: 'Nicky Pe',
    title: 'NEWS & RESEARCH',
    content: [
        ''
    ],
    author: null,
    button: null,
    buttonLink: null
};

export const RESOURCE_PAGE_VIDEO_BG = {
    video: resourceBg,
    videoSource: 'Shah Jahan',
    title: 'RESOURCES',
    content: [
        ''
    ],
    author: null,
    button: null,
    buttonLink: null
};

export const GLOSSARY_PAGE_VIDEO_BG = {
    video: glossaryBg,
    videoSource: 'Михайло Пожега',
    title: 'GLOSSARY',
    content: [
        'The A-Z of biodiversity.'
    ],
    author: null,
    button: null,
    buttonLink: null
};


export const STORY_PAGE_VIDEO_BG = {
    video: storiesBg,
    videoSource: 'dabatepatvideos',
    title: 'STORIES',
    content: [
        'Bite-sized, scrollable looks into the challenges facing biodiversity around the world.',
        "Check back for new stories, or get in touch via our feedback form if you have your own story to tell about our planet's wildlife!"
    ],
    author: null,
    button: null,
    buttonLink: null
};

export const IMPERVIOUS_VIDEO_BG = {
    video: imperviousBg,
    videoSource: null,
    title: 'IMPERVIOUS',
    content: [
        'How our sprawling cities lead the attack on wildlife.'
    ],
    author: 'Douglas Kissack',
    button: null,
    buttonLink: null
};